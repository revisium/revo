...
describe('embedded PostgreSQL SQL readiness', () => {
  ...
  it(
    'reports bad database authentication without leaking connection details',
    async () => {
  ...
  it(
    'close cancels and drains only the readiness SQL operation',
    async () => {
  ...
  it(
    'enforces the server-side statement timeout for blocked readiness SQL',
    async () => {
  ...
  it(
    'converts a terminated blocked SQL connection into a safe owned failure',
    async () => {
  ...
  it(
    'rejects repeated close when transport cannot confirm SQL settlement by the deadline',
    async () => {
      const failure = {
        name: 'EmbeddedPostgresError',
        message: 'Embedded PostgreSQL preparation failed',
